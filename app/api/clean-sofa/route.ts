...
import { processSofaImageTransformation } from "@/lib/api/processors/sofa-processor";
import { uploadToCloudinary, getOptimizedImageUrl } from "@/lib/cloudinary";
import type { CleanImageResponse } from "@/types";
import type { SofaRenderMode } from "@/lib/types/sofa-types";
...
/**
 * POST /api/clean-sofa
 * Endpoint spécialisé pour le nettoyage de canapés et salons
 */
export async function POST(request: NextRequest) {
  const startTime = Date.now();
  console.log("🛋️ Nouvelle requête clean-sofa");
  ...
  try {
    ...
    // 5. Mode de rendu canapé
    const renderMode =
      (fields.renderMode?.[0] as SofaRenderMode) || "professional-clean";
    console.log("🎨 Mode de rendu canapé:", renderMode);

    // Valider le mode
    const validModes: SofaRenderMode[] = ["professional-clean", "magazine-worthy", "designer-dream"];
    ...
    // 6. Transformation IA canapé
    console.log("🤖 Utilisation du système de nettoyage canapé");
    console.log("🎨 Mode:", renderMode);
    console.log("💰 Coût estimé: $0.039 par image");
    console.log("🤖 Démarrage de la transformation canapé...");
    const { generatedImage, analysis, spaceType } = await processSofaImageTransformation(
      processedBuffer,
      renderMode
    );
    console.log("✅ Transformation canapé terminée");
    console.log("🛋️ Type d'espace détecté:", spaceType);
    ...
    const uploadResult = await uploadToCloudinary(generatedImage, {
      folder: "clear-ai/sofa",
      publicId: `sofa_${renderMode}_${timestamp}`,
    });
    ...
    console.error("❌ ERREUR CANAPÉ:", err);
    ...
    const errorMessage =
      err?.message || "Une erreur est survenue lors de la génération canapé";
    const errorDetails =
